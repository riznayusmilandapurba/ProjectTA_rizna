...
<body onload="window.print()">
	...
	<?php $dataSession = $this->session->all_userdata();
			...
			if ($id_user->role_id == 2) {
				$kelas = $this->db->query("SELECT tbl_data_kelas.nama_kelas, tbl_data_kelas.id_pengajar ,tbl_data_siswa.id_kelas,tbl_data_siswa.id_user FROM tbl_data_siswa
	...
	<?php
			...
			if ($ajar->semester == "Genap") {
				if ($rata_rata >= 70) {
					$kelasAwal = $kelas->id_kelas + 1;
					if ($kelasAwal < 7) {
						$this->db->query("UPDATE tbl_data_siswa SET id_kelas=$kelasAwal WHERE id_siswa=$siswa->id_siswa");
						$message = "Selamat Anda Naik Ke Kelas " . $kelasAwal;
					} else {
						$dataAlumni = $this->db->query("SELECT COUNT(*) as jumlah FROM tbl_album_alumni WHERE id_user=$kelas->id_user")->row();
						if ($dataAlumni->jumlah < 1) {
							$this->db->query("INSERT INTO tbl_album_alumni(id_user,nama_siswa,nisn,tahun_ajar) VALUES('$kelas->id_user','$siswa->nama_siswa','$siswa->nisn','$ajar->tahun_ajar')");
						}
						$message = "Selamat Anda Telah Lulus Dan Anda Telah Menjadi Alumni";
					}
				} else {
